...
const toValueAndUnit = (range: DataRange): ValueAndUnit | null => {
  switch (range) {
    case "5m":
      return [5, "minutes"];
    case "30m":
...
const calcStart = (end: Dayjs, range: DataRange): Dayjs | null => {
  const r = toValueAndUnit(range);
  if (r === null) {
    return null;
  }
  const [value, unit] = r;
  return end.subtract(value, unit);
};
...
export const SystemCharts = () => {
  ...
  const handleRangeChange = (event: ChangeEvent<HTMLInputElement>) => {
    const end = dayjs();
    const newRange = event.target.value as DataRange;
    setChartConfig({
      ...chartConfig,
      start: calcStart(end, newRange)!,
      range: newRange,
      end,
    });
  };
  ...
  return (
    <>
      <div className="flex flex-col">
        <div className="flex flex-row">
          <div className="join">
            ...
            />
            <button
              className={`join-item btn ${chartConfig.range === "Custom" ? "btn-primary" : ""}`}
              disabled={isLoading}
              popoverTarget="custom-dr-popover"
              popoverTargetAction="show"
              style={{
                anchorName: "--custom-dr-anchor",
              }}
            >
              Custom
              <Calendar />
      ...
      <DataRangeDropdown
        ...
        positionAnchor="--custom-dr-anchor"
        onApply={(s) => {
          setChartConfig({
            ...chartConfig,
            start: s.start,
